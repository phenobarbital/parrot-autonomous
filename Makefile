# Nav-AGENT Makefile
# This Makefile provides a set of commands to manage the Nav-AGENT project.
...
venv:
	...
	@echo 'run `source .venv/bin/activate` to start develop with Nav-AGENT.'
...
# Format code
format:
	uv run black Nav-AGENT

# Lint code
lint:
	uv run pylint --rcfile .pylint Nav-AGENT/*.py
	uv run black --check Nav-AGENT

# Run tests with coverage
test:
	uv run coverage run -m Nav-AGENT.tests
	uv run coverage report
	uv run mypy Nav-AGENT/*.py
...
bump-patch:
	@python -c "import re; \
	content = open('Nav-AGENT/version.py').read(); \
	version = re.search(r'__version__ = \"(.+)\"', content).group(1); \
	parts = version.split('.'); \
	parts[2] = str(int(parts[2]) + 1); \
	new_version = '.'.join(parts); \
	new_content = re.sub(r'__version__ = \".+\"', f'__version__ = \"{new_version}\"', content); \
	open('Nav-AGENT/version.py', 'w').write(new_content); \
